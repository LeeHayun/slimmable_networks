...
def dataset(train_transforms, val_transforms, test_transforms):
    if not FLAGS.test_only:
        train_set = datasets.CIFAR100(FLAGS.dataset_dir, train=True, transform=train_transforms)
    else:
        train_set = None
    val_set = datasets.CIFAR100(FLAGS.dataset_dir, train=False, transform=val_transforms)
    test_set = None
